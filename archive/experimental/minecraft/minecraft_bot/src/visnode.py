...
def get_block_multi(coords):
    ...
    try:
        getBlocksFromSrv = rospy.ServiceProxy(
            'get_block_multi', get_block_multi_srv)
...
def handle_get_visible_blocks(req):
    ...
    coords = vis.get_coordinates_in_range(
        req.x, req.y, req.z, req.pitch, req.yaw)
