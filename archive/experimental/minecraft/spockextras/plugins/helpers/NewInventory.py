...
class InventoryPlugin(PluginBase):
    ...
    def set_slot(self, window_id, slot_nr, slot_data):
        ...
        self.emit_set_slot(slot,self.inventory.window)

    def emit_set_slot(self, slot, window):        
        to_be_emitted= {'window_id':window.window_id,'window_title':window.title, 'slot_count':len(window.slots), 'slots':[]}
